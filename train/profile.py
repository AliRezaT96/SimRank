...
def profile_model(model_name):
    model = model.create_model(model_name, force_custom_text=True)
    model.eval()
    ...
    model_cfg = model.get_model_config(model_name)
    if model_cfg:
        vision_cfg = model.CLIPVisionCfg(**model_cfg['vision_cfg'])
        text_cfg = model.CLIPTextCfg(**model_cfg['text_cfg'])
